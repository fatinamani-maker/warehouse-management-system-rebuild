...
const AuditLogPage = () => {
    const columns = [
        { key: "timestamp", label: "Time", sortable: true, render: (r) => new Date(r.timestamp).toLocaleString() },
        { key: "userName", label: "User", sortable: true },
        { key: "action", label: "Action", render: (r) => <Badge variant="outline" className="text-[10px] capitalize">{r.action}</Badge> },
        { key: "entity", label: "Entity" },
        { key: "entityId", label: "Entity ID", render: (r) => <span className="font-mono text-xs">{r.entityId}</span> },
        { key: "details", label: "Details" },
        { key: "tenantId", label: "Tenant", render: (r) => <span className="font-mono text-xs">{r.tenantId}</span> },
    ];
    return (<AppLayout>
