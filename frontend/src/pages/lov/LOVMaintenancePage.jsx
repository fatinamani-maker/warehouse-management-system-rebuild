...
const LOVMaintenancePage = () => {
    ...
    const columns = [
        ...
        { key: "values", label: "Values", render: (r) => <span className="text-xs">{r.values.length} items</span> },
        { key: "usedIn", label: "Used In", render: (r) => r.usedIn.map((u) => <Badge key={u} variant="outline" className="text-[10px] mr-1">{u}</Badge>) },
    ...
    return (<AppLayout>
      ...
      {selected && (<Card className="mt-4">
          ...
          <CardContent>
            <div className="space-y-1.5">
              {selected.values.map((v, i) => (<div key={i} className="flex items-center justify-between py-1.5 px-3 rounded border text-sm hover:bg-accent/50">
                  <span>{v}</span>
      ...
      <ConfirmModal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} onConfirm={() => { toast({ title: "Deleted (mock)", description: `Removed: ${deleteConfirm}` }); setDeleteConfirm(null); }} title="Delete LOV Value" description={`Are you sure you want to delete "${deleteConfirm}"? This may affect screens that use this value.`}/>
