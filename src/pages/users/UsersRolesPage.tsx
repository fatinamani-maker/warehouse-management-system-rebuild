...
const UsersRolesPage = () => {
  ...
  const userColumns = [
    ...
    { key: "role", label: "Role", render: (r: any) => <Badge variant="outline" className="text-xs">{r.role}</Badge> },
    { key: "status", label: "Status", render: (r: any) => <StatusBadge status={r.status} /> },
    { key: "lastLogin", label: "Last Login", render: (r: any) => new Date(r.lastLogin).toLocaleDateString() },
  ];
            ...
            <Card className="mt-4">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-1.5">
                  {rolePermissions[viewAsRole]?.map(p => (
          ...
          <Card>
            <CardContent className="pt-4 overflow-x-auto">
              <table className="text-xs w-full">
                ...
                <tbody>
                  {permissions.map(p => (
                    <tr key={p} className="hover:bg-accent/50">
